...
function getCatByTitle(titulo) {
  ...
  if (t.includes('panel ac') || t.includes('maleta') || t.includes('travel') || t.includes('cuadro') || t.includes('alfombra')) return 'pcat_01KREC2BTSDBBBWF4NE9FMZPZ9Q';
...
async function main() {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    if (res.status === 200 || res.status === 201) {
      ...
      errores++;
      console.log(`❌ [${i+1}/${products.length}] ${sku}: ${JSON.stringify(res.data?.message || res.data)}`);
    }
